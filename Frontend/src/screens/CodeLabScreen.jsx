...
const PRESET_TASKS = [
  {
    title: 'Train a simple linear regression',
...
def information_gain(X, y, feature_idx, threshold):
    ...
    child_entropy = (sum(left_mask)/n)*left_entropy + (sum(right_mask)/n)*right_entropy
...
print(f"Centroids:\\n{centroids}")`,
  ...
  {
    title: 'Write a neural network forward pass',
    code: `import numpy as np
    ...
    <div className="min-h-screen bg-bg">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex items-center justify-between mb-6 animate-fade-in-up">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-text-muted">Write, run, and review ML code in your browser</p>
            </div>
          </div>
          <div className="flex items-center gap-1.5">
            <div className={`w-2 h-2 rounded-full ${pyodideReady ? 'bg-[#2E7D32]' : 'bg-[#E65100] animate-pulse'}`} />
            <span className="text-xs text-text-muted">
              {pyodideReady ? 'Python Ready' : pyodideLoading ? 'Loading Python...' : 'Offline'}
            </span>
          </div>
        </div>
        ...
        <div className="mb-4 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
          <label className="section-label block mb-2">Select Task</label>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
          ...
          <div className="flex flex-col">
            <div className="flex items-center justify-between px-4 py-2 bg-surface rounded-t-xl border border-b-0 border-border">
              <span className="text-xs text-text-muted font-mono">editor.py</span>
              <div className="flex gap-1.5">
                <div className="w-3 h-3 rounded-full bg-[#C62828]/40" />
                <div className="w-3 h-3 rounded-full bg-[#E65100]/40" />
                <div className="w-3 h-3 rounded-full bg-[#2E7D32]/40" />
              </div>
            </div>
            <textarea
              value={code}
              ...
              spellCheck={false}
              className="flex-1 min-h-[400px] p-4 bg-[#1A1A1A] border border-border rounded-b-xl font-mono text-sm text-[#F5F2ED] resize-none focus:outline-none focus:border-black transition-colors leading-relaxed"
            />
          </div>
          ...
          <div className="flex flex-col">
            <div className="flex items-center justify-between px-4 py-2 bg-surface rounded-t-xl border border-b-0 border-border">
              <span className="text-xs text-text-muted font-mono">output</span>
              <span className="section-label">stdout</span>
            </div>
            <div className="flex-1 min-h-[400px] p-4 bg-[#0D0D0D] border border-border rounded-b-xl font-mono text-sm text-[#C8FF00] whitespace-pre-wrap overflow-auto">
        ...
        <div className="flex flex-wrap gap-3 mt-4">
          <button
            ...
            className="px-6 py-2.5 rounded-xl bg-accent text-black text-sm font-bold hover:bg-accent-dark transition-all disabled:opacity-50 cursor-pointer flex items-center gap-2"
          >
            {running ? (
              <>
          ...
          <button
            ...
            disabled={loading}
            className="px-6 py-2.5 rounded-xl bg-black text-white text-sm font-medium hover:bg-[#1F1F1F] transition-all disabled:opacity-50 cursor-pointer flex items-center gap-2"
          >
            {loading ? (
              <>
        ...
        {evalResult && (
          <div className="mt-4 p-5 rounded-xl bg-card border border-border animate-fade-in-up">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-text-primary">AI Code Review</h3>
              <span className={`text-2xl font-bold font-display ${getScoreColor(evalResult.score)}`}>
                {evalResult.score}/10
              </span>
            </div>
            {/* Score bar */}
            <div className="w-full h-2 rounded-full bg-surface mb-4">
              <div
                className={`h-2 rounded-full transition-all duration-1000 ${
                  evalResult.score >= 8 ? 'bg-[#2E7D32]' : evalResult.score >= 5 ? 'bg-[#E65100]' : 'bg-[#C62828]'
                }`}
            ...
            <div className="space-y-3">
              <div>
                <h4 className="section-label mb-1">Feedback</h4>
                <p className="text-sm text-text-secondary">{evalResult.feedback}</p>
              </div>
              {evalResult.fix && (
                <div>
                  <h4 className="section-label mb-1">Suggestion</h4>
                  <p className="text-sm text-text-secondary">{evalResult.fix}</p>
                </div>
              )}
            </div>
          </div>
